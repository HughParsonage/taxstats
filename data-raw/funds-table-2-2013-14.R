...
GET(url = "http://data.gov.au/dataset/25e81c18-2083-4abe-81b6-0f530053c63f/resource/68ac57dd-1660-4aa0-be5a-f0b9a51064a2/download/taxstats2014fund2smsfselecteditemsbyyear.xlsx", 
...
funds_table2_smsf_201314 <- 
  ...
  {
    dot <- .
    kill <- which(sapply(dot, function(v) all(is.na(v))))
    select(dot, -kill)
  } %>%
...
devtools::use_data(funds_table2_smsf_201314)
